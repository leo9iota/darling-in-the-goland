...
- [x] Create asset loading utilities
  ...
  - [x] Font loader: load TTF from disk → `text/v2` face *(implemented in `internal/gui/text.go`)*
...
- [x] Load `public-pixel-font.ttf` as a reusable font face *(via `gui.LoadFont()`)*
...
- [x] Create `internal/physics/collision.go`
  ...
  - [ ] One-way platform logic *(deferred → FEAT-002)*
- [x] Write unit tests
...
- [ ] Level transition logic *(deferred → FEAT-002)*
...
- [x] Create entity manager
  ...
  - [x] `GetCounts()`: returns entity counts for debug overlay
...
- [x] Create `internal/gui/scale.go`: centralized font size constants (8/16/24px)
- [x] Final verification
  - [x] `go vet ./...`: zero warnings
  - [x] `go build ./cmd/game`: produces single executable
  - [x] Full playthrough of all levels start to finish
  - [x] All GUI elements functional (HUD, pause menu, debug overlay)
...
- [x] Initialize Go module: `go mod init github.com/leo9iota/darling-in-the-goland`
- [x] Add Ebitengine dependency: `go get github.com/hajimehoshi/ebiten/v2`
- [x] Create `cmd/game/main.go` with minimal `ebiten.Game` implementation
- [x] Configure window: 1280×720, title "Darling in the GoLand", pixel-art scaling
- [x] Set `ebiten.SetVsyncEnabled(false)` to match current `conf.lua` (vsync = 0)
- [x] Verify: window opens, solid background color, clean exit on ESC
...
- [x] Create `internal/core/background.go`: port `Background.lua`
...
- [x] Create `internal/animation/sprite.go`: generic frame-based animation
  - Frame list, timer, rate, current frame tracking
  - Horizontal flip via `GeoM.Scale(-1, 1)`
- [x] Set up asset embedding or loading from disk (`os.Open` / `ebitenutil.NewImageFromFile`)
- [x] Load `public-pixel-font.ttf` via `text/v2` (Ebitengine font API)
- [x] Verify: parallax background scrolls when simulating camera movement with arrow keys
...
- [x] Create `internal/physics/world.go`
  - Global gravity vector (0, 1500)
  - `Update(dt)`: apply gravity, integrate velocity, broadphase, narrowphase
  - Collision callbacks: `OnBeginContact`, `OnEndContact`
- [x] Create `internal/physics/collision.go`
  - Contact struct: body pair, normal, penetration depth
  - Position correction (push bodies apart)
  - One-way platform support *(deferred → FEAT-002)*
- [x] Verify: a test body falls under gravity and lands on a static platform
...
- [x] ~~Export existing Tiled maps to `.tmj`~~ — parsed `.tmx` (XML) directly instead
- [x] Create `internal/tilemap/loader.go`
  - Parse `.tmx` XML: layers, tilesets, objects
...
- [x] Create `internal/core/camera.go`: port `Camera.lua`
  - Position, target, smoothing (damped spring), bounds, scale (2×)
  - `Follow(x, y)`, `Update(dt)`, apply via `GeoM` on draw
- [x] Compute `MapWidth` from ground layer for camera bounds
- [x] Verify: the first level renders at 2× scale, camera follows a moving point
...
- [x] Create `internal/entity/player.go`: port `Player.lua`
...
- [x] Create `internal/entity/coin.go`: port `Coin.lua`
...
- [x] Create `internal/entity/spike.go`: port `Spike.lua`
  - Static sensor body, deals 1 damage on player contact
- [x] Create `internal/entity/stone.go`: port `Stone.lua`
  - Dynamic body, pushed by player
- [x] Create `internal/entity/enemy.go`: port `Enemy.lua`
...
- [x] Entity manager: `UpdateAll(dt)`, `DrawAll(screen)`, `RemoveAll()`, `GetCounts()`
  - Separate slices for each entity type
- [x] Map spawning: read entity layer objects, instantiate by `type` field
- [x] Verify: all entity types spawn from the map, interact with player, and behave as expected
...
- [x] Create `internal/gui/hud.go`: port `HUD.lua`
  - Heart counter: draw `N` hearts based on `player.Health.Current`
  - Coin counter: coin icon + count with pixel font
  - Shadow effect: draw at offset in black at 50% opacity
- [x] Create `internal/gui/menu.go`: port `Menu.lua`
...
- [x] Create `internal/gui/debug.go`: port `DebugGUI.lua`
  - FPS, frame time (ms), memory usage
  - Entity counts (coins, enemies, spikes, stones, total)
  - Toggle with F3
- [x] Create `internal/gui/scale.go`: centralized font size constants
- [x] Verify: full playthrough, `go vet ./...` clean, `go build ./cmd/game` works
...
5. ~~No Lua source files remain in the repository.~~ Lua source kept in `docs/code/` for reference.
